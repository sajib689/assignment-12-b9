...
const ScholarDetails = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const reviewsCollection = { universityId, universityName, rating, review };
   
    axiosPublic.post("/reviews", reviewsCollection)
      .then((res) => {
      ...
      <div className="container mx-auto space-y-12 mt-12 mb-24">
        <div className="flex flex-col overflow-hidden rounded-md shadow-sm lg:flex-row">
          ...
          <div className="flex flex-col justify-center flex-1 ps-6 dark:bg-gray-50">
            <span className="text-xs uppercase dark:text-gray-600">Join Here</span>
            <h3 className="text-3xl font-bold">{universityName}</h3>
            <p className="dark:text-gray-600">Scholarship category: {scholarshipCategory}</p>
            <p className="dark:text-gray-600">University location: {country}</p>
            <p className="dark:text-gray-600">Application Deadline: {applicationDeadline}</p>
            <p className="dark:text-gray-600">Scholarship Description: {scholarshipDescription}</p>
            ...
            <p className="dark:text-gray-600">Service Charge: ${serviceCharge}</p>
            <p className="dark:text-gray-600">Application Fees: ${applicationFees}</p>
            <button
              type="button"
